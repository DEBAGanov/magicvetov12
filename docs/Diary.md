...
## 2026-06-24 - Оптимизация PageSpeed страницы /catalog (CLS, картинки, SEO, a11y)

### Проблемы (отчёт PageSpeed от 23.06.2026)
- Производительность 67 (моб) / 77 (десктоп). Главная просадка — **CLS 0.459** (норма <0.1) и избыточный вес изображений (~3.5 МБ лишнего).
- LCP 10.4 с на мобиле: LCP-картинка помечена `loading="lazy"`, без `fetchpriority`.
- `canonical` страницы `/catalog` вёл на корень домена (наследовался от root layout).
- A11y 89: нет `<main>` landmark; недостаточный контраст серого текста в футере; ссылки-иконки в шапке без доступного имени.

### Наблюдения
- CLS вызван не футером как таковым, а тем, что `/catalog` — клиентский компонент: статический prerender показывал крошечный fallback «Загрузка...», затем 6 скелетонов, затем 12 карточек. Трёхступенчатое изменение высоты роняло футер вниз — Lighthouse атрибутировал сдвиг футеру (`bg-gray-900`).
- `ProductCard` рендерил native `<img>` ВСЕХ изображений товара сразу (по 4-5 на карточку) → лишние загрузки. В `next.config.ts` стоял `unoptimized: true` — Next не ресайзил 1000×1000 под контейнер ~321px и не отдавал srcset/AVIF.

### Решения
- `next.config.ts`: включена оптимизация изображений (`formats: avif/webp`, `imageSizes`/`deviceSizes` под сетку каталога, `minimumCacheTTL` 31 день). Убран `unoptimized: true`.
- `ProductCard.tsx`: переход с `<img>` на `next/image` (`fill` + `sizes="(max-width:768px) 50vw, 33vw"`). Доп. изображения карусели монтируются только после первого наведения (`activated`) — не грузятся для всех видимых карточек. Добавлен проп `priority` для первого экрана.
- `catalog/page.tsx`: первым 4 карточкам передаётся `priority` (eager + fetchpriority high для LCP). Скелетон приведён к реальной высоте карточки (12 шт = размер страницы), Suspense-fallback повторяет каркас страницы — устраняет сдвиги при гидрации.
- `catalog/layout.tsx`: добавлен `alternates.canonical = "/catalog"`.
- Root `layout.tsx`: `<div className="flex-1">` → `<main className="flex-1">`.
- `Footer.tsx`: `text-gray-500` → `text-gray-400` (контраст). `Header.tsx`: `aria-label` на ссылки-логотип и корзину.
- `npx tsc --noEmit` и `npm run build` — успешно.

### Открытые вопросы
- **Проверить на staging перед деплоем**: включение оптимизации изображений направляет все картинки через `/_next/image` на Node-сервере (standalone). Нужно убедиться, что прод-прокси (nginx) проксирует `/_next/image` и что в сборке есть `sharp`. Откат — одна строка (`unoptimized: true`).
- Заголовок `Cache-Control` для картинок на S3 (`s3.twcstorage.ru`) — экономия ~4 МБ при повторных визитах — настраивается на бакете Timeweb S3, а не в коде Next. С включённой Next-оптимизацией оригиналы с S3 кешируются оптимизатором, но прямые ссылки на S3 всё ещё без долгого кэша.
...
### Решения
- Созданы 5 лендингов категорий с полным SEO:
  - /rozy — Розы с доставкой (title, keywords, FAQ, SEO-текст)
  - /tyulpany — Тюльпаны с доставкой
  - /cvety-v-korobke — Цветы в коробке
  - /korziny-s-cvetami — Корзины с цветами
  - /nedorogie-cvety — Недорогие цветы от 990 ₽
- Созданы 2 сезонных лендинга:
  - /na-den-materi — День матери
  - /na-novyj-god — Новый год
- Обновлён sitemap.ts: 7 новых страниц (5 категорий + 2 сезона)
- Обновлён Footer.tsx: добавлены ссылки на все категории (Розы, Тюльпаны, В коробке, Корзины, Недорогие)
- Все страницы используют OccasionLandingContent с FAQ Schema.org и BreadcrumbList

### Наблюдения
- Итого: 38 маршрутов, 15 статей блога, 9 лендингов поводов/сезонов, 5 лендингов категорий, 2 городских лендинга
- robots.txt на продакшене работает корректно (200 OK)

### Проблемы
- Нет проблем — сборка прошла успешно

## 2026-04-25 - Реализация Фазы 3 SEO: Локальное SEO

### Решения
- Обновлён Footer.tsx: добавлен NAP (Name, Address, Phone) для Волжска и Зеленодольска в структурированном формате
- Добавлен кросс-линкинг в футере: разделы «Доставка цветов» (ссылки на городские лендинги), «Поводы» (все 7 поводов), «Информация» (блог, FAQ, контакты)
- Исправлен AggregateRating в floristSchema: 156 → 6 (реальное число отзывов)
- Добавлена Review Schema.org на страницу /otzyvy — каждый отзыв получает свою разметку Review
- Добавлены OG-теги на страницу /otzyvy (title, description, locale)
- Добавлен кросс-линкинг в CityLandingContent: блок «Цветы на любой повод» с 8 ссылками на лендинги поводов + ссылка на соседний город
- Добавлены 5 новых SEO-статей блога:
  1. Красные розы: значение и как выбрать (krasnye-rozy-znachenie-i-kak-vybrat)
  2. 101 роза: королевский подарок (101-roza-korolevskij_podarok)
  3. Хризантемы — долгоиграющие цветы (hrizantemy-dolgoigrajushhie-cvety)
  4. Цветы маме на день рождения (cvety-mame-na-den-rozhdeniya)
  5. Букеты до 2000 рублей (bukety-do-2000-rublej)
- Добавлена ссылка «Блог» в Header (десктоп + мобильное меню) вместо FAQ
- Добавлены ссылки «Блог» и «Вопросы и ответы» в Footer
- BreadcrumbList Schema.org добавлена на все информационные страницы (o-nas, kontakty, otzyvy, dostavka)

### Наблюдения
- Яндекс.Бизнес и Google My Business уже зарегистрированы пользователем
- Ниша доставки цветов в Зеленодольске/Волжске практически свободна
- Всего 15 SEO-статей блога покрывают основные информационные запросы

### Проблемы
- robots.txt всё ещё может отдавать 403 на продакшене — нужно проверить nginx

## 2026-04-25 - Реализация Фазы 2 SEO: Контент и структура

### Решения
- Создан блог с 10 SEO-статьями (blog-articles.ts): доставка в городах, розы, тюльпаны, пионы, значение цветов, свадебные букеты, цветы в коробке, день рождения
- Создана страница /blog с grid-раскладкой и BreadcrumbList schema
- Создана страница /blog/[slug] с Article schema, related articles, CTA
- Создана страница /faq с 15+ вопросами в 5 категориях, FAQPage Schema.org
- Обновлён sitemap.ts: добавлены страницы блога и FAQ

## 2026-04-25 - Реализация Фазы 1 SEO-оптимизации

### Решения
- Обновлён H1 на главной: добавлены города «Доставка цветов по Волжску и Зеленодольску»
- Обновлён layout.tsx: добавлен og:image (1200x630), manifest.webmanifest, theme-color, twitter images, улучшен title/description с ценами «от 990 ₽»
- Обновлён next.config.ts: добавлены poweredByHeader: false, compress: true, trailingSlash: false, security headers (X-Frame-Options, X-Content-Type-Options, Referrer-Policy), кеширование статики
- Исправлен sitemap.ts: lastModified теперь фиксированные даты вместо new Date()
- Обновлён robots.ts: добавлен /search в disallow
- Исправлена опечатка в constants.ts: «Открыта» → «Открытка»
- Создан manifest.webmanifest (PWA)
- Созданы favicon.ico (32x32), apple-touch-icon.png (180x180), og-image.jpg (1200x630) из SVG
- Обновлены Title/Description ВСЕХ 14 посадочных страниц:
  - Городские (Волжск, Зеленодольск): цены от 990 ₽, УТП «фото ДО оплаты», расширенные keywords
  - 7 лендингов поводов: добавлен бренд «Магия Цветов», эмодзи, цены, OG tags
  - 4 информационные страницы: добавлены keywords, улучшены descriptions
- Обновлены city descriptions в constants.ts с УТП и эмодзи

### Наблюдения
- H1 на главной уже был в HeroBanner.tsx — добавлены города для локального SEO
- Сборка Next.js прошла успешно — все 24 маршрута без ошибок
- sharp установлен для генерации изображений из SVG (нужно удалить из node_modules при деплое)

### Проблемы
- robots.txt всё ещё может отдавать 403 на продакшене — нужно проверить конфигурацию nginx
- sharp добавлен как зависимость — перед деплоем выполнить `npm prune`

## 2026-04-25 - Глубокий SEO-анализ конкурентов и создание стратегии

### Наблюдения
- Проведён анализ 3 ключевых конкурентов: flowwow.com, volzhsk.flowlove.ru, cvety.kz и 7 других
- Flowwow.com: главная заблокирована от индексации (`noindex, nofollow`), городские страницы на английском
- FlowLove (volzhsk.flowlove.ru): единственный реальный конкурент во Волжске, 700+ городов через поддомены, WordPress, нет Schema.org
- Из 10 проверенных конкурентов 3 домена истекли, 3 возвращают ошибки — ниша свободна
- Ниша доставки цветов в Зеленодольске и Волжске практически не занята онлайн-конкурентами
- Сайт magiacvetov12.ru на 75-80% готов к SEO, но есть критические пробелы

### Решения
- Создан комплексный SEO-план: `docs/SEO_STRATEGY_PLAN.md`
- Определены 20 ключевых запросов для Зеленодольска и Волжска (низкая конкуренция)
- Спланированы 4 фазы оптимизации: критические исправления → контент → локальное SEO → продвинутые техники
- Рекомендована подкаталоговая структура (уже реализована через /dostavka-cvetov/volzhsk, zelenodolsk)
- Уникальное преимущество: Schema.org 6 типов (ни у одного конкурента нет)

### Проблемы
- robots.txt отдаёт 403 на продакшене — блокирует краулеры
- Нет H1 на главной странице
- Нет дефолтного og:image — при шеринге пустая превью
- lastModified в sitemap всегда текущая дата
- Каталог — клиентский рендеринг (плохо для Яндекса)
- Опечатка в constants.ts: «Открыта» вместо «Открытка»

## 2026-04-15 - Завершение Next.js 15 фронтенда

### Наблюдения
- Все 8 этапов плана реализованы успешно
- Сборка Next.js проходит без ошибок — 13 маршрутов
- First Load JS ~102kB (shared), страницы 2-4kB каждая
- standalone Docker build обеспечивает минимальный образ

### Решения
- Исправлен баг в API клиенте: `getUserOrders` возвращал `ProductPage` вместо `OrderPage`
- Metadata для client-компонентов вынесена в отдельные layout.tsx файлы
- Добавлен `OrderPage` тип для пагинации заказов

### Проблемы
- Нет проблем — все этапы завершены

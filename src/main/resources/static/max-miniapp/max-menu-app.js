...
class PizzaNatMaxMenuApp {
            ...
            productElement.innerHTML = `
                ...
                <div class="menu-item-info">
                    ...
                    <div class="menu-item-actions">
                        ${quantity === 0 ?
                            `<button class="add-button ${product.preorder ? 'preorder-button' : ''}" data-product-id="${product.id}">${product.preorder ? 'под заказ' : 'добавить'}</button>` :
    ...
    openImageViewer(product) {
        ...
        if (viewer && img && title && price) {
            ...
            if (addBtn) {
                addBtn.textContent = product.preorder ? 'Под заказ' : 'Добавить в корзину';
            }
